...
const s = {
  ...
  header: { display: "flex", justifyContent: "space-between", gap: "18px", alignItems: "flex-start", marginBottom: "22px" },
  ...
  h1: { margin: 0, color: "#334052", fontSize: "32px", lineHeight: 1.1 },
  intro: { margin: "8px 0 0", color: "#5D554B", fontSize: "16px", lineHeight: 1.45 },
  field: { display: "grid", gap: "6px", fontWeight: 800, color: "#4B443C", fontSize: "14px" },
  input: { width: "100%", boxSizing: "border-box", border: "1px solid #D2C4B3", borderRadius: "12px", padding: "10px 12px", fontSize: "15px", background: "#FBF7EF", color: "#443E37" },
  textarea: { width: "100%", boxSizing: "border-box", border: "1px solid #D2C4B3", borderRadius: "12px", padding: "10px 12px", fontSize: "15px", lineHeight: 1.4, background: "#FBF7EF", color: "#443E37", minHeight: "92px" },
  actions: { display: "flex", flexWrap: "wrap", gap: "10px", marginTop: "14px" },
  button: { border: "1px solid #D2C4B3", borderRadius: "999px", minHeight: "38px", padding: "8px 14px", background: "#E8DDCC", color: "#334052", fontWeight: 900, cursor: "pointer" },
  mainButton: { border: "1px solid #7F8A69", borderRadius: "999px", minHeight: "38px", padding: "8px 14px", background: "#7F8A69", color: "white", fontWeight: 900, cursor: "pointer" },
  link: { display: "inline-flex", alignItems: "center", justifyContent: "center", minHeight: "38px", padding: "8px 14px", borderRadius: "999px", background: "#E8DDCC", color: "#334052", textDecoration: "none", fontWeight: 900, border: "1px solid #D2C4B3" },
  info: { margin: "6px 0", color: "#5D554B", lineHeight: 1.4 },
  scoreBox: { background: "#E8DDCC", border: "1px solid #D2C4B3", borderRadius: "16px", padding: "16px", marginTop: "14px" },
  scoreNumber: { margin: "0 0 6px", fontSize: "28px", fontWeight: 900, color: "#334052" },
  checkboxLine: { display: "flex", gap: "10px", alignItems: "flex-start", padding: "10px 0", color: "#4B443C", fontWeight: 800 },
...
export function DossierPersonnePage() {
  ...
  function ajouterJournal() {
    ...
    const entree = { id: String(Date.now()), date: dateNote, type: typeNote, texte: nouvelleNote.trim(), createdAt: new Date().toISOString() };
  ...
  function copierSynthese() {
    ...
    setMessage("Synthèse dossier copiée.");
  }
      ...
      <div style={s.wrap}>
        <header style={s.header}>
          <div>
            ...
            <p style={s.intro}>Suivi du dossier sans affichage du nom complet. Données enregistrées localement dans ce navigateur.</p>
          </div>
        ...
        <section style={s.card}>
          ...
          <label style={{ ...s.field, marginTop: "14px" }}>Nouvelle note<textarea style={s.textarea} value={nouvelleNote} onChange={(event) => setNouvelleNote(event.target.value)} /></label>
          <button style={s.mainButton} type="button" onClick={ajouterJournal}>Ajouter au journal</button>
          <div style={{ marginTop: "18px" }}>
            {journal.length === 0 ? <p style={s.info}>Aucune note journalisée pour le moment.</p> : journal.map((item) => <article key={item.id} style={{ ...s.card, marginBottom: "10px", padding: "14px" }}><p style={{ ...s.info, margin: 0 }}><strong>{item.date} — {item.type}</strong></p><p style={{ ...s.info, marginBottom: 0 }}>{item.texte}</p></article>)}
          </div>
        </section>

        <section style={s.card}>
          <p style={s.label}>Synthèse dossier</p>
          <textarea style={{ ...s.textarea, minHeight: "250px" }} readOnly value={synthese} />
          <div style={s.actions}>
            <button style={s.mainButton} type="button" onClick={copierSynthese}>Copier la synthèse</button>
